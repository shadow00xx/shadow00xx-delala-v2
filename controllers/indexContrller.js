...
exports.store = (req, res) => { res.render('pages/stores', { title: 'متاجر' }) }
exports.test = (req, res) => { res.render('test', { title: 'test', layout:"layouts/chat" }) }
exports.serveces = (req, res) => { res.render('pages/serveces', { title: 'خدمات' }) }
exports.terms = (req, res) => { res.render('pages/terms', { title: 'الشروط والاحكام' }) }
exports.about = (req, res) => { res.render('pages/about', { title: 'من نحن' }) }
...
exports.exploreCategories = async (req, res) => {
    ...
    } catch (error) { console.log(error); res.render('error/500' ) }
}
...
exports.exploreCategoriesById = async (req, res) => {
    try {
        ...
        const limitNumber = 20;
        const categoryById = await prodects.find({ 'category': categoryId }).limit(limitNumber).sort({ createdAt: 'desc' })
...
exports.addcontact = async(req, res)=>{
    try {
        const connt = { contact:req.body.contact, createby:req.user.id }
        const contact = await new contacts(connt)
        await contact.save(contact)
        req.flash('success_msg',' نشكرك علي التواصل معنا' );
        res.redirect('/');
    } catch (err) { console.log(err); res.render('error/500' ) }
}
